...
const judgeFamily = (): 'claude' | 'gemini' =>
  JUDGE_MODEL.startsWith('claude') ? 'claude' : 'gemini';

/**
 * One cache file per judge model.
 *
 * Keying only on session id would let a cross-family run overwrite the
 * same-family results, destroying the comparison the second run exists to
 * make. The two are different instruments and their readings are kept apart.
 */
const CACHE = join(
  homedir(),
  'flowlog-baseline',
  `judge-cache.${JUDGE_MODEL}.json`,
);
/** The original run, before per-model cache files existed. */
const LEGACY_CACHE = join(homedir(), 'flowlog-baseline', 'judge-cache.json');
...
async function callClaude(prompt: string): Promise<string> {
  ...
  if (!key)
    die('ANTHROPIC_API_KEY is not set. Add it to .env or the environment.');
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      model: JUDGE_MODEL,
      max_tokens: 2048,
      // Judging, not writing. Deterministic so a re-run of the same cue cannot
      // produce a different verdict and quietly move the score.
      temperature: 0,
...
async function main(): Promise<void> {
  ...
  // Fall back to the pre-per-model cache so the original run is not re-billed.
  const cachePath =
    existsSync(CACHE) || judgeFamily() !== GENERATOR_FAMILY
      ? CACHE
      : LEGACY_CACHE;
  const cache: Record<string, CueJudgement> =
    !has('fresh') && existsSync(cachePath)
      ? JSON.parse(readFileSync(cachePath, 'utf8'))
      : {};

  const sameFamily = judgeFamily() === GENERATOR_FAMILY;
  ...
  console.error(
    `\n  judging ${subjects.length} cues with ${JUDGE_MODEL}` +
      `\n  cues were generated by ${GENERATOR_FAMILY} — ` +
      `${sameFamily ? 'SAME FAMILY, see the caveat in the report' : 'CROSS-FAMILY'}\n`,
  );
  ...
  console.log(
    sameFamily
      ...
      : `  Cross-family run: cues by ${GENERATOR_FAMILY}, judged by ${JUDGE_MODEL}.\n`,
  );
